...
def upsert_regional(conn, nombre: str):
    ...
    conn.commit()
    return conn.execute("SELECT id FROM regional WHERE nombre=?", (nombre,)).fetchone()[0]
...
def search_moleculas(conn, texto: str, solo_activas=True, limit=400):
...
def get_or_create_pedido_periodo(conn, periodo: str, usuario: str=""):
    ...
    conn.execute("INSERT INTO pedido(id, periodo, creado_en, usuario, estado) VALUES(?,?,?,?, 'EN_CURSO')",
                 (pid, periodo, pd.Timestamp.utcnow().isoformat(), usuario))
    conn.commit()
    return pid
...
with c1:
    y = st.selectbox("Año", years, index=years.index(hoy.year))
with c2:
    m = st.selectbox("Mes", months, index=hoy.month-1)
...
def list_regionales(conn): return [r[0] for r in conn.execute("SELECT nombre FROM regional ORDER BY nombre").fetchall()]
def list_eron_by_regional(conn, regional_nombre: str):
    return [r[0] for r in conn.execute("""
        ...
        WHERE regional.nombre = ?
        ORDER BY eron.nombre
    """, (regional_nombre,)).fetchall()]

with tab_add:
    if not pid:
        st.info("Primero **crea/abre** el pedido del periodo.")
    else:
    ...
    if not pid:
        ...
        if items.empty:
            ...
            edit_df = st.data_editor(
                ...
                key="carrito_editor_pro_nodel",
            )
            # Persist inline changes
            ...
            if st.button(f"🗑️ Eliminar seleccionados ({len(to_delete)})", disabled=len(to_delete)==0):
